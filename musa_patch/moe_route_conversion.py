...
try:
    ...
    if version.parse(triton.__version__) < version.parse("3.4.0") and not torch.musa.is_available():
        HAVE_TRITON = False
    else:
...
@triton.jit
def _route_indices_to_multihot_kernel(
    indices_ptr,
...
@triton.jit
def _route_multihot_to_indices_kernel(
    probs_in_multihot_ptr,
    position_map_ptr,
    probs_indices_ptr,
    num_of_local_experts: tl.constexpr,
...
class IndicesToMultihot(torch.autograd.Function):
    ...
    def forward(ctx, indices, probs_indices, num_of_local_experts):
        ...
        Returns:
            ...
            probs_in_multihot: [num_of_tokens, num_of_local_experts]
        '''
        assert indices.is_contiguous() and probs_indices.is_contiguous()
        assert probs_indices.dtype == torch.float32
        assert indices.dtype in (torch.int32, torch.int64)
        assert indices.device == probs_indices.device
        ...
        multihot_indices = torch.empty(
            (num_of_tokens, num_of_local_experts), dtype=torch.bool, device=indices.device
        )
        probs_in_multihot = torch.empty(
            (num_of_tokens, num_of_local_experts), dtype=probs_indices.dtype, device=indices.device
        )
        position_map = torch.empty(
            (num_of_tokens, num_of_local_experts), dtype=torch.int32, device=indices.device
        )
        ...
        grid = (num_of_tokens,)
        _route_indices_to_multihot_kernel[grid](
            indices,
            probs_indices,
            multihot_indices,
            probs_in_multihot,
    ...
    def backward(ctx, grad_multihot_indices, grad_probs_in_multihot):
        ...
        grid = (num_of_tokens,)
        _route_multihot_to_indices_kernel[grid](
...
def install():
    """Patch only Flex DeepEP's indices conversion, never ACE or other dispatchers.

    DeepEP supplies unique valid local expert indices or -1 padding per row.
    Inputs outside the tested layout/type contract retain the original path.
    """
    from megatron.core.transformer.moe.token_dispatcher import _DeepepManager

    original = _DeepepManager._indices_to_multihot
    if getattr(original, "_musa_route_fused", False):
        return

    def convert(self, indices, probs):
        supported = (
            HAVE_TRITON and indices.device.type == "musa"
            and indices.dtype == torch.int64 and probs.dtype == torch.float32
            and probs.device == indices.device
            and indices.ndim == 2 and probs.shape == indices.shape
            and indices.shape[0] > 0 and indices.shape[1] > 0
            and self.num_local_experts > 0
            and indices.is_contiguous() and probs.is_contiguous()
        )
        if not supported:
            return original(self, indices, probs)
        return fused_indices_to_multihot(indices, probs, self.num_local_experts)

    convert._musa_route_fused = True
    _DeepepManager._indices_to_multihot = convert
