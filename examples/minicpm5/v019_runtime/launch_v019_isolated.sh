...
RUN=${ROOT}/v019_compat_30step_20260915/${V019_ATTEMPT}
...
mkdir -p "$RUN"
printf '%s\n' "$RUN" > "${ROOT}/v019_compat_30step_20260915/latest_run.txt"
...
for host in "${HOSTS[@]}"; do
  ...
  if grep -q ':30619 ' "$RUN/preflight.${host}.log"; then
...
cleanup() {
  ...
  trap - EXIT
  for host in "${HOSTS[@]}"; do
    "${SSH[@]}" root@"$host" bash "${ROOT}/v019_node_supervisor.sh" stop "$V019_ATTEMPT" "$host" || true
  done
...
pids=()
for host in "${HOSTS[@]}"; do
  "${SSH[@]}" root@"$host" bash "${ROOT}/v019_node_supervisor.sh" run "$V019_ATTEMPT" "$host" \
