...
if [[ "$mode" == stop ]]; then
  if [[ -f "$PIDFILE" ]]; then
    ...
    if [[ "$job" =~ ^[0-9]+$ && -r /proc/$job/cmdline ]] && \
       tr '\0' ' ' < /proc/$job/cmdline | grep -q '/mbzz_ssd/modelbest_v0.19/run_v019_node_isolated.sh' && \
...
[[ "$mode" == run ]] || exit 2
setsid timeout --signal=TERM --kill-after=30s 10800 \
  bash ${ROOT}/run_v019_node_isolated.sh &
job=$!
printf '%s\n' "$job" > "$PIDFILE"
(
