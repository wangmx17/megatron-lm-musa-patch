...
TRAINING_ARGS+=(--optimizer "${OPTIMIZER:-muon}" --muon-momentum 0.9  --muon-coefficient-type quintic  --muon-num-ns-steps 5 --muon-scale-mode spectral --muon-extra-scale-factor 1.0 --muon-tp-mode blockwise --muon-scalar-optimizer adam)
...
if [[ "${ENABLE_DEEPEP:-0}" -eq 1 ]]; then
  ...
  export DEEPEP_ACE_DYNAMIC_TOKEN_NUM="${DEEPEP_ACE_DYNAMIC_TOKEN_NUM:-1}"
  export MCCL_CROSS_NIC="${MCCL_CROSS_NIC:-1}"
  # Dynamic mode derives local token capacity from the first dispatch. Static
  # values remain available for an explicit DEEPEP_ACE_DYNAMIC_TOKEN_NUM=0 fallback.
...
echo "[deepep_ace][input] env DYNAMIC_TOKEN_NUM=${DEEPEP_ACE_DYNAMIC_TOKEN_NUM:-unset} TOKEN_NUM=${DEEPEP_ACE_TOKEN_NUM:-unset} HIDDEN=${DEEPEP_ACE_HIDDEN:-unset} NUM_TOPK=${DEEPEP_ACE_NUM_TOPK:-unset} NUM_BUFFERS=${DEEPEP_ACE_NUM_BUFFERS:-unset}"
...
if [[ "${V019_VALIDATE_ONLY:-0}" -eq 1 ]]; then
  export WORLD_SIZE=16 RANK=0 LOCAL_RANK=0
  python "${PRETRAIN_FILE}" "${MODEL_ARGS[@]}" "${MOE_ARGS[@]}" "${TRAINING_ARGS[@]}" "${REGULARIZATION_ARGS[@]}" "${LEARNING_RATE_ARGS[@]}" "${MODEL_PARALLEL_ARGS[@]}" "${MIXED_PRECISION_ARGS[@]}" "${DATA_ARGS[@]}" "${LOGGING_ARGS[@]}" "${TRANSFORMER_ENGINE_ARGS[@]}" "${LOAD_ARGS[@]}"
  exit $?
fi
"${cmd[@]}"
