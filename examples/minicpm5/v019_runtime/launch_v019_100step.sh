...
mkdir -p "$RUN"
printf '%s\n' "$RUN" > "${ROOT}/v019_compat_100step_loss_scale_2p32_20260915/latest_run.txt"
...
cleanup() {
  ...
  trap - EXIT
  for host in "${HOSTS[@]}"; do
    "${SSH[@]}" root@"$host" bash "${ROOT}/v019_100step_node_supervisor.sh" stop "$V019_ATTEMPT" "$host" || true
  done
...
pids=()
for host in "${HOSTS[@]}"; do
  "${SSH[@]}" root@"$host" bash "${ROOT}/v019_100step_node_supervisor.sh" run "$V019_ATTEMPT" "$host" \
