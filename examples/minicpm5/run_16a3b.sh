...
export USE_DEEPEP_ACE=${USE_DEEPEP_ACE:-${ENABLE_DEEPEP}}
...
export ENABLE_ACE_WGRAD_OVERLAP=${ENABLE_ACE_WGRAD_OVERLAP:-0}
...
export MUSA_TE_THD_LSE_FP32=${MUSA_TE_THD_LSE_FP32:-require}
export NVTE_MUSA_THD_CP_CORRECTION_FUSION=${NVTE_MUSA_THD_CP_CORRECTION_FUSION:-0}
...
for binary_flag in ENABLE_DEEPEP USE_DEEPEP_ACE ENABLE_ACE_WGRAD_OVERLAP; do
  if [[ "${!binary_flag}" != "0" && "${!binary_flag}" != "1" ]]; then
    echo "Error: ${binary_flag} must be 0 or 1, got ${!binary_flag}."
    exit 2
  fi
done
if [[ "${USE_DEEPEP_ACE}" == "1" && "${ENABLE_DEEPEP}" != "1" ]]; then
  echo "Error: USE_DEEPEP_ACE=1 requires ENABLE_DEEPEP=1."
  exit 2
fi
if [[ "${ENABLE_ACE_WGRAD_OVERLAP}" == "1" && "${USE_DEEPEP_ACE}" != "1" ]]; then
  echo "Error: ENABLE_ACE_WGRAD_OVERLAP=1 requires USE_DEEPEP_ACE=1."
  exit 2
fi
...
STACK:                 current branch (DeepEP=${ENABLE_DEEPEP}, ACE=${USE_DEEPEP_ACE}, bias-SwiGLU fusion on)
WORK_HOME:             ${WORK_HOME}
...
USE_DEEPEP_ACE:        ${USE_DEEPEP_ACE:-0}
ENABLE_ACE_WGRAD_OVERLAP: ${ENABLE_ACE_WGRAD_OVERLAP:-0}
