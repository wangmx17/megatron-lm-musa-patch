...
# MiniCPM5 16A3B / MTT S5000 4-node 32-GPU launcher.
...
HOSTFILE=${HOSTFILE:-"${WORK_HOME}/hostfile_4node"}
...
# This launcher connects to every node over SSH.  The container starts with
# `sleep infinity`, so make sure its SSH endpoint is ready before preflight.
mkdir -p /run/sshd
ss -ltn | grep -q ":${SSH_PORT} " || /usr/sbin/sshd -p "${SSH_PORT}"
...
RDZV_ID=${RDZV_ID:-"minicpm5_4node_old_stack_${CURRENT_TIME}"}
...
cat <<CFG
================ MiniCPM5 16A3B S5000 4-node 32-GPU launch ==================
...
DEEPEP_ACE_DYNAMIC_TOKEN_NUM: ${DEEPEP_ACE_DYNAMIC_TOKEN_NUM}
...
MUON_BATCH_NS_MAX_B:   ${MUON_BATCH_NS_MAX_B:-unset}
MUON_DP1_LOW_MEMORY:   ${MUON_DP1_LOW_MEMORY:-unset}
MUON_FUSED_POINTWISE:  ${MUON_FUSED_POINTWISE:-unset}
...
ENABLE_CE_TE='${ENABLE_CE_TE:-0}' \
ENABLE_DEEPEP='${ENABLE_DEEPEP:-0}' \
DEEPEP_ACE_DYNAMIC_TOKEN_NUM='${DEEPEP_ACE_DYNAMIC_TOKEN_NUM:-1}' \
ENABLE_MANUAL_GC='${ENABLE_MANUAL_GC:-0}' \
ENABLE_ROPE_FUSION='${ENABLE_ROPE_FUSION:-0}' \
ENABLE_MOE_ROUTER_FUSION='${ENABLE_MOE_ROUTER_FUSION:-0}' \
ENABLE_DEEPEP_ENV='${ENABLE_DEEPEP_ENV:-0}' \
...
MUON_FUSED_POINTWISE='${MUON_FUSED_POINTWISE:-0}' \
