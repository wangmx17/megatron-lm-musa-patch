...
run_training_on_this_node() {
  ...
  export ENABLE_RMSNORM_FUSION=0 USE_GROUPED_GEMM=1 MOE_FLEX_DISPATCHER_NUM_SMS=56
  ...
  export MUSA_TE_PADDED_METADATA_NO_SYNC=0
  export MUSA_FUSED_ROUTE_CONVERSION=${MUSA_FUSED_ROUTE_CONVERSION:-1}
  export MUON_TE_EXPERT_BATCH_NS=0 ENABLE_FLEX_SHARED_EXPERT_EAGER=0
  export ENABLE_SHARED_EXPERT_OVERLAP=0 ENABLE_PROFILER=0
  # Backward CP overlap only. The forward half runs its P2P next to forward
  # attention and inflates that kernel by more than the hidden comm is worth.
